    ...
    public static void onCommonSetup() {
        ForgeRegistries.BIOMES.forEach(biome -> {
            if (biome.getCategory() == Biome.Category.NETHER) {
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, SoulstoneFeature.INSTANCE.configure(
                        new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NETHERRACK, ModBlocks.SOULSTONE.get().getDefaultState(), 64)
                ).createDecoratedFeature(Placement.COUNT_RANGE.configure(new CountRangeConfig(4, 0, 0, 128))));
            } else if (biome.getCategory() == Biome.Category.THEEND) {
                // the end
            } else {
                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.configure(
                        new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, ModBlocks.PROSPERITY_ORE.get().getDefaultState(), 8)
                ).createDecoratedFeature(Placement.COUNT_RANGE.configure(new CountRangeConfig(12, 0, 0, 50))));

                biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.configure(
                        new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, ModBlocks.INFERIUM_ORE.get().getDefaultState(), 8)
                ).createDecoratedFeature(Placement.COUNT_RANGE.configure(new CountRangeConfig(16, 0, 0, 50))));
            }
        });
    }
}
